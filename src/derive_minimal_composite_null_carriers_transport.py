...
THEOREM_ID = "minimal_composite_null_carriers_transport"
...
def carrier_profile(carrier: list[int], profiles: dict[int, dict[str, Any]]) -> dict[str, Any]:
    rows = [profiles[sector] for sector in carrier]
...
def classification_for(carrier: list[int]) -> tuple[str, str]:
    if carrier == [25, 26]:
        return (
    ...
    if carrier == [6, 26]:
        return (
            "mixed_S_channel_public_zero_superselection_carrier_isolated_from_pi33",
    ...
    if carrier == [33]:
        return (
            "primitive_support_exact_residual_carrier",
            "unique primitive public-zero carrier selected by the certified sector-33 height transport",
        )
    return (
...
def build_theorem() -> dict[str, Any]:
    ...
    primitive_record = carrier_profile(PRIMITIVE_CARRIER, profiles)
    primitive_record["self_transport"] = transport_rank(PRIMITIVE_CARRIER, PRIMITIVE_CARRIER, profiles)
    primitive_record["classification"], primitive_record["classification_reason"] = classification_for(
        PRIMITIVE_CARRIER
    )

    records = []
    for carrier in TARGET_MINIMAL_COMPOSITES:
        record = carrier_profile(carrier, profiles)
        record["self_transport"] = transport_rank(carrier, carrier, profiles)
        record["transport_to_pi33"] = transport_rank(carrier, PRIMITIVE_CARRIER, profiles)
        record["transport_from_pi33"] = transport_rank(PRIMITIVE_CARRIER, carrier, profiles)
        ...
        record["classification"], record["classification_reason"] = classification_for(carrier)
    ...
    non_pi33_minimal_supports = [support for support in minimal_supports if support != PRIMITIVE_CARRIER]
    ...
    checks = {
        ...
        is True,
        "boolean_public_zero_carriers_match_upstream": boolean_public_zero_supports
        ...
        "minimal_composite_self_transport_ranks_are_nonzero": {
            ",".join(map(str, record["carrier"])): record["self_transport"]["rank"]
        ...
        "mixed_s_channel_overlap_does_not_imply_pi33_transport": next(
            record for record in records if record["carrier"] == [6, 26]
        )["object_channel_overlap_with_pi33"]
        == ["S+"]
        and next(record for record in records if record["carrier"] == [6, 26])["couples_to_pi33"]
        is False,
    ...
    status = (
        ...
        if all_checks_pass
        else "D20_MINIMAL_COMPOSITE_NULL_CARRIERS_TRANSPORT_NEEDS_REVIEW"
    )

    report = {
        "schema": "d20.theorem.minimal_composite_null_carriers_transport.source_drop",
        "status": status,
        "object": "d20",
        "claim": (
            "The two minimal non-Pi_33 public-zero composite idempotent carriers have nonzero self-transport "
            "and zero mixed transport to Pi_33. Thus neither is gauge zero; both are public-zero "
            "superselection carriers isolated from the primitive residual carrier Pi_33."
        ),
        "definition": {
            ...
            "not_gauge_zero": "A public-zero carrier with positive self-transport rank and positive permutation rank.",
            "isolated_from_pi33": "Both E_C A985 e_33 and e_33 A985 E_C have rank zero.",
            "superselection_null_carrier": (
                "A public-zero carrier with nonzero internal transport that is isolated from the primitive "
                "height-residual carrier."
            ),
        },
        "inputs": {
        ...
        "derived": {
            "primitive_carrier_pi33": primitive_record,
            "minimal_non_pi33_composite_carriers": TARGET_MINIMAL_COMPOSITES,
            ...
            "transport_components": {
                ...
                "mixed_S_channel_superselection_null_carrier": [6, 26],
            },
            ...
            "transport_to_pi33_ranks": {
                ",".join(map(str, record["carrier"])): record["transport_to_pi33"]["rank"]
                for record in records
            },
            "transport_from_pi33_ranks": {
                ",".join(map(str, record["carrier"])): record["transport_from_pi33"]["rank"]
                for record in records
            },
        },
        "interpretation": {
            ...
            "mixed_S_channel_carrier": (
                "{6,26} is a mixed S-/S+ public-zero superselection carrier: one M2 block plus one scalar "
...
def write_theorem(out_dir: Path = DEFAULT_OUT_DIR) -> dict[str, Any]:
    ...
    manifest = {
        "schema": "d20.theorem.minimal_composite_null_carriers_transport_manifest.source_drop",
        "name": THEOREM_ID,
        ...
        "certification_tests": [
            "consume the certified Boolean public-zero idempotent carrier enumeration",
            "verify the two minimal non-Pi_33 composite supports are {6,26} and {25,26}",
            "apply the central-sector transport rank formula to the composite carriers",
            ...
            "classify the composites as isolated public-zero superselection carriers, not gauge zero",
        ],
    }
