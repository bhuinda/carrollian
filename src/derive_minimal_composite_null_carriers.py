#!/usr/bin/env python3
"""
Classify D20 minimal composite public-zero sector idempotent carriers.
...
--admissibility-report: optional report.json from
            sector_idempotent_carrier_admissibility theorem.

Outputs
-------
- minimal_composite_null_carriers_profile_certificate.json
- minimal_composite_null_carriers_profile_report.md
...
carrier enumeration. It classifies the two minimal non-Pi33 public-zero composites
...
def load_public_zero_carriers(path: str | None) -> Tuple[List[List[int]], Dict[str, Any]]:
    if not path:
        return DEFAULT_PUBLIC_ZERO_CARRIERS, {"source": "built_in_from_user_certified_statement"}
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    # Be deliberately permissive about report schema names.
    candidates = None
    for key in [
        "public_zero_carriers",
        "zero_public_carriers",
        "boolean_public_zero_carriers",
        "carriers",
    ]:
    ...
    if candidates is None:
        ...
        for key in ["result", "results", "theorem", "certificate"]:
            ...
            if isinstance(sub, dict):
                for kk in ["public_zero_carriers", "zero_public_carriers", "carriers"]:
                    if kk in sub:
                        candidates = sub[kk]
                        break
            if candidates is not None:
                break
    if candidates is None:
        candidates = DEFAULT_PUBLIC_ZERO_CARRIERS
...
def carrier_profile(profiles: List[Dict[str, Any]], carrier: List[int]) -> Dict[str, Any]:
    ps = [profile_for(profiles, s) for s in carrier]
    ...
    # For central primitive blocks, the internal algebra dimension of the carrier is sum d_i^2.
    ...
    if carrier == PRIMITIVE_CARRIER:
        ...
        classification = "public_zero_superselection_carrier"
...
def make_report(cert: Dict[str, Any]) -> str:
    lines = []
    lines.append("# D20 minimal composite null carrier classification")
    lines.append("")
    lines.append(f"Status: `{cert['status']}`")
    lines.append("")
    lines.append("## Public-zero carrier set")
    lines.append("")
    lines.append("The classified Boolean public-zero carrier set is:")
    lines.append("")
    for c in cert["public_zero_carriers"]:
        lines.append(f"- `{c}`")
    lines.append("")
    lines.append("## Minimal composite carriers")
    lines.append("")
    lines.append("| carrier | classification | active objects | block dims | internal dim | permutation rank | verdict |")
    lines.append("|---|---|---|---|---:|---:|---|")
    for rec in cert["minimal_composite_classification"]:
        verdict = "not gauge; public-null internal carrier"
        lines.append(
            f"| `{rec['carrier']}` | `{rec['classification']}` | "
    ...
    lines.append("")
    lines.append("`{33}` is the unique primitive public-zero carrier and remains the support-exact sector-33 height-transport carrier.")
    lines.append("")
    lines.append("The two minimal non-Pi33 public-zero composites are not gauge zeros because each has nonzero internal algebra dimension and nonzero permutation rank. The carrier `{25,26}` is a pure `S-` superselection null doublet. The carrier `{6,26}` is a mixed `S-/S+` hidden boundary-null composite.")
    lines.append("")
...
def main() -> None:
    ...
    public_zero, pz_meta = load_public_zero_carriers(args.admissibility_report)

    classifications = [carrier_profile(profiles, c) for c in public_zero if c]
    minimal = [r for r in classifications if r["carrier"] in TARGET_MINIMAL_COMPOSITES]
    primitive = [r for r in classifications if r["carrier"] == PRIMITIVE_CARRIER]

    # Assertions matching the theorem state.
    assert sorted(public_zero, key=lambda x: (len(x), x)) == sorted(DEFAULT_PUBLIC_ZERO_CARRIERS, key=lambda x: (len(x), x)), public_zero
    ...
    cert = {
        "schema": "d20.minimal_composite_null_carriers.profile.source_drop",
        "status": "D20_MINIMAL_COMPOSITE_NULL_CARRIERS_PROFILE_CLASSIFIED",
        "input": {
        ...
        "public_zero_carriers": public_zero,
        "minimal_composite_classification": minimal,
        "primitive_carrier_record": primitive[0],
        "all_nonzero_public_zero_classification": classifications,
        "theorem": {
            "primitive_carrier": [33],
            "minimal_non_pi33_composites": TARGET_MINIMAL_COMPOSITES,
            "statement": "{33} is the unique primitive public-zero carrier; {25,26} is a pure S- superselection null doublet; {6,26} is a mixed S-channel hidden boundary-null composite. Both composites are not gauge zeros because they have nonzero internal algebra dimension and nonzero permutation rank.",
        },
    }
    cert["certificate_sha256"] = canonical_json_hash(cert)

    cpath = os.path.join(args.out, "minimal_composite_null_carriers_profile_certificate.json")
    rpath = os.path.join(args.out, "minimal_composite_null_carriers_profile_report.md")
