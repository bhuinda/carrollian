...
THEOREM_ID = "sector_idempotent_carrier_admissibility"
...
def build_theorem() -> dict[str, Any]:
    ...
    checks = {
        ...
        == [[6, 26], [25, 26], [33]],
        "certified_height_transport_support_exact_carrier_is_only_33": height_support_exact_supports
        == [[33]]
    ...
    status = (
        ...
        if all_checks_pass
        else "D20_SECTOR_IDEMPOTENT_CARRIER_ADMISSIBILITY_NEEDS_REVIEW"
    )

    report = {
        "schema": "d20.theorem.sector_idempotent_carrier_admissibility.source_drop",
        "status": status,
        "object": "d20",
        "claim": (
            "In the orthogonal sector-idempotent algebra, public-zero idempotent carriers are exactly the "
            "six Boolean sector sums listed in this certificate, including zero. There are five nonzero "
            "public-zero boundary-null idempotent carriers, so public-zero idempotence alone does not make "
            "Pi_33 unique. Pi_33 is unique only under primitive/single-sector support, and it is the only "
            "support-exact carrier for the already-certified sector-33 height transport."
        ),
        "definition": {
            ...
            "public_zero_idempotent_carrier": (
                "A Boolean sector sum whose combined A42/A12 public shadow is zero."
            ),
            "boundary_null": (
                "The carrier annihilates all 30 directed boundary-to-loop pair projections on both left and right."
            ),
            "height_support_exact": (
                "The carrier support is exactly the certified carrier sector support {33} used by the all-residue "
        ...
        "derived": {
            ...
            "height_support_exact_carriers_for_certified_transport": height_support_exact_supports,
            "candidate_rows_sha256": sha_json(candidate_rows),
            "candidate_rows": candidate_rows,
        },
        "interpretation": {
            "what_survives": (
                "Pi_33 remains the unique primitive/single-sector public-zero carrier and the unique support-exact "
                "carrier for the certified height transport."
            ),
            "what_fails": (
                "The stronger claim that Pi_33 is the only public-zero idempotent carrier is false: two "
                "non-Pi_33 composites, {6,26} and {25,26}, are also public-zero boundary-null idempotents."
            ),
            "admissibility_boundary": (
                "Future physics statements must specify whether carriers are primitive, support-exact, or allowed "
        ...
        "next_highest_yield_item": (
            "Classify the observable meaning of the two non-Pi_33 null composite carriers {6,26} and {25,26}: "
...
def write_theorem(out_dir: Path = DEFAULT_OUT_DIR) -> dict[str, Any]:
    ...
    manifest = {
        "schema": "d20.theorem.sector_idempotent_carrier_admissibility_manifest.source_drop",
        "name": THEOREM_ID,
        ...
        "certification_tests": [
            ...
            "separate primitive/single-sector support from composite null carriers",
            "identify the unique support-exact carrier for the certified sector-33 height transport",
        ],
    }
