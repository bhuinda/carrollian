...
C2_CUBICAL_AGDA_SELECTOR_FINITE_SUBTYPE_SINGLETONS_REPORT = (
    D20_INVARIANTS
    / "theorems"
...
CUBICAL_AGDA_SOURCE = (
    ...
    / "formal"
    / "cubical"
    / "C2SelectorFoundationSelectorFiniteSubtypeLazy63.agda"
)
CUBICAL_AGDA_INTERFACE = CUBICAL_AGDA_SOURCE.with_suffix(".agdai")

LAZY_SELECTOR_KEY = "lazy_componentwise_spectral_gap"
LAZY_SELECTOR_AGDA = "lazyComponentwiseSpectralGap"
LAZY_MEMBERSHIP_PREFIX = "lazyGapMember"
LAZY_COUNT = 63
LAZY_SPEC = SelectorFiniteSubtypeSpec(
    module_name="C2SelectorFoundationSelectorFiniteSubtypeLazy63",
    selector_key=LAZY_SELECTOR_KEY,
    selector_agda=LAZY_SELECTOR_AGDA,
    prefix="lazySpectralGap",
    membership_prefix=LAZY_MEMBERSHIP_PREFIX,
    fiber_count=LAZY_COUNT,
    index_type_name="LazySpectralGapSelectorIndex",
    fin_pattern_style="call_suc",
    wrap_fin_lhs=True,
)
...
def lazy_fiber(bridge: dict[str, Any]) -> dict[str, Any]:
    return selected_fiber(bridge, LAZY_SELECTOR_KEY, LAZY_COUNT)


def generate_agda_source(bridge: dict[str, Any]) -> str:
    return generate_selector_finite_subtype_agda(bridge, LAZY_SPEC)
...
def build_theorem() -> dict[str, Any]:
    ...
    singletons = load_json(C2_CUBICAL_AGDA_SELECTOR_FINITE_SUBTYPE_SINGLETONS_REPORT)
    ...
    fiber = lazy_fiber(selector_payload)
    ...
    source_summary = {
        ...
        "module": "C2SelectorFoundationSelectorFiniteSubtypeLazy63",
        "typecheck_command": (
        ...
        "proof_counts": {
            ...
            "to_fin_data_selected_clause_count": len(
                re.findall(
                    r"^lazySpectralGapFiberToFinData \(d\d+ , lazyGapMember\d+\) = ",
                    source_text,
                    re.MULTILINE,
                )
            ),
            "to_fin_data_impossible_clause_count": len(
                re.findall(
                    r"^lazySpectralGapFiberToFinData \(d\d+ , \(\)\)$",
                    source_text,
                    re.MULTILINE,
                )
            ),
            "from_fin_data_clause_count": len(
                re.findall(
                    r"^lazySpectralGapFiberFromFinData .+ = d\d+ , lazyGapMember\d+$",
                    source_text,
                    re.MULTILINE,
                )
            ),
            "right_inverse_clause_count": len(
                re.findall(
                    r"^lazySpectralGapFiberFinDataRightInv .+ = refl$",
                    source_text,
                    re.MULTILINE,
                )
            ),
            "left_inverse_selected_clause_count": len(
                re.findall(
                    r"^lazySpectralGapFiberFinDataLeftInv \(d\d+ , lazyGapMember\d+\) = refl$",
                    source_text,
                    re.MULTILINE,
                )
            ),
            "left_inverse_impossible_clause_count": len(
                re.findall(
                    r"^lazySpectralGapFiberFinDataLeftInv \(d\d+ , \(\)\)$",
                    source_text,
                    re.MULTILINE,
                )
            ),
            "fin_equivalence_count": len(
                re.findall(
                    r"^lazySpectralGapFiberEquivFin : SelectorFiber lazyComponentwiseSpectralGap ≃ Fin 63$",
                    source_text,
                    re.MULTILINE,
                )
            ),
    ...
    checks = {
        ...
        "singleton_finite_subtype_theorem_is_certified": singletons.get("status")
        == "D20_FULL_EXPOSURE_ZERO_PAIR_SOURCED_BALANCE_C2_CUBICAL_AGDA_SELECTOR_FINITE_SUBTYPE_SINGLETONS_CERTIFIED"
        and singletons.get("all_checks_pass") is True,
        "actual_c2_kernel_fixed_orbits_drive_lazy_selector": len(selected_ids) == LAZY_COUNT
        and dynamics_count == 543,
        "lazy_source_defines_sigma_subtype": (
            "SelectorFiber s = Σ[ d ∈ DynamicsId ] SelectorMembership s d" in source_text
        ),
        "lazy_source_has_exact_63_fiber_witnesses": counts[
            "selected_count"
        ]
        ...
        == LAZY_COUNT,
        "lazy_source_has_exact_480_impossible_rows": counts[
            "to_fin_data_impossible_clause_count"
        ]
        == counts["left_inverse_impossible_clause_count"]
        == nonselected_count
        == 480,
        "lazy_source_has_fin63_equivalence": counts["fin_equivalence_count"] == 1
        and "lazySpectralGapFiberEquivFin = compEquiv lazySpectralGapFiberFinDataEquiv (FinData≃Fin 63)"
        in source_text,
        "lazy63_agda_interface_artifact_present_after_typecheck": interface_is_fresh,
        "lazy63_artifact_hashes_are_stable": sha_file(CUBICAL_AGDA_SOURCE)
    ...
    status = (
        ...
        if all_checks_pass
        else "D20_FULL_EXPOSURE_ZERO_PAIR_SOURCED_BALANCE_C2_CUBICAL_AGDA_SELECTOR_FINITE_SUBTYPE_LAZY63_NEEDS_REVIEW"
    )
    report = {
        "schema": "d20.theorem.full_exposure_zero_pair_sourced_balance_c2_cubical_agda_selector_finite_subtype_lazy63",
        "status": status,
        "object": "d20",
        "claim": (
            "The lazy componentwise spectral-gap selector fiber is packaged as a Cubical Agda Sigma subtype "
            "and equipped with a typechecked equivalence to Fin 63."
        ),
        "inputs": {
            ...
            "c2_cubical_agda_selector_finite_subtype_singletons_report": {
                "path": rel(C2_CUBICAL_AGDA_SELECTOR_FINITE_SUBTYPE_SINGLETONS_REPORT),
                "sha256": sha_file(C2_CUBICAL_AGDA_SELECTOR_FINITE_SUBTYPE_SINGLETONS_REPORT),
            },
            "cubical_agda_selector_finite_subtype_lazy63_source": {
                "path": rel(CUBICAL_AGDA_SOURCE),
                "sha256": sha_file(CUBICAL_AGDA_SOURCE),
            },
            "cubical_agda_selector_finite_subtype_lazy63_interface": {
                "path": rel(CUBICAL_AGDA_INTERFACE),
                "sha256": sha_file(CUBICAL_AGDA_INTERFACE) if interface_exists else None,
            },
        },
        ...
        "interpretation": {
            "what_this_proves": [
                "the lazy componentwise spectral-gap ambiguity is a finite Cubical Agda subtype equivalent to Fin 63",
                "the proof uses generated membership constructors for the 63 selected dynamics ids",
                "the other 480 dynamics ids are discharged by explicit impossible rows",
            ],
            "what_remains": [
                "the paired lazy spectral-gap fiber of size 480",
                "the raw spectral-gap fiber of size 543",
                "a compact shared generator for non-singleton finite subtype proofs",
            ],
        },
        "checks": checks,
        "all_checks_pass": all_checks_pass,
        "next_highest_yield_item": (
            "Generate and typecheck the 480-element paired lazy selector finite-subtype module, reusing "
            "the explicit impossible-row pattern."
        ),
    }
