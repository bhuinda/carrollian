...
def validate_eta6_p18() -> dict[str, Any]:
    expected = build_payloads()
    report = load_json(OUT_DIR / "report.json")
    manifest = load_json(OUT_DIR / "manifest.json")
    p18 = load_json(OUT_DIR / "p18.json")
    ...
    if p18 != expected["p18"]:
        raise AssertionError("eta6_p18 JSON mismatch")
    if cert != expected["cert"]:
        raise AssertionError("eta6_p18 cert mismatch")
    if top_csv != expected["top_csv"]:
        raise AssertionError("eta6_p18 top CSV mismatch")
    if obs_csv != expected["obs_csv"]:
        raise AssertionError("eta6_p18 obs CSV mismatch")
    if not np.array_equal(np.asarray(tables["top_table"]), expected["top_table"]):
        raise AssertionError("eta6_p18 top table mismatch")
    if not np.array_equal(
        np.asarray(tables["observable_table"]),
        expected["obs_table"],
    ):
        raise AssertionError("eta6_p18 observable table mismatch")

    if report.get("schema") != "eta6.p18.report@1":
        raise AssertionError("eta6_p18 report schema mismatch")
    if report.get("status") != STATUS:
        raise AssertionError("eta6_p18 report is not certified")
    if report.get("all_checks_pass") is not True:
        raise AssertionError("eta6_p18 all_checks_pass is not true")
    if report.get("checks") != expected["report"]["checks"]:
        raise AssertionError("eta6_p18 checks mismatch")
    if self_hash(report, "certificate_sha256") != report.get("certificate_sha256"):
        raise AssertionError("eta6_p18 report self hash mismatch")
    if report.get("certificate_sha256") != expected["report"]["certificate_sha256"]:
        raise AssertionError("eta6_p18 report hash mismatch")
    ...
    if tuple(top_table.shape) != (TOP_N, len(TOP_COLUMNS)):
        raise AssertionError("eta6_p18 top table shape mismatch")
    if tuple(obs_table.shape) != (len(OBS_CODES), len(OBS_COLUMNS)):
        raise AssertionError("eta6_p18 obs table shape mismatch")
    ...
    required = {
        ...
        "grid_cell_count": 300178,
        "raw_lookup_count": 61647945,
        "raw_pair_count": 28758081,
        "outside_raw_pair_count": 26546089,
        "inside_222_raw_pair_count": 2211992,
    ...
    for key, value in required.items():
        if obs.get(OBS_CODES[key]) != value:
            raise AssertionError(f"eta6_p18 observable {key} mismatch")

    best = table_rows(top_table, TOP_COLUMNS)[0]
    if [best[f"p5_{letter}"] for letter in "abcdef"] != [6, 12, 19, 29, 94, 96]:
        raise AssertionError("eta6_p18 best p5 ids mismatch")
    if best["joint_support_spread"] != 1164096:
        raise AssertionError("eta6_p18 best spread mismatch")
    if top_csv.splitlines()[0].split(",") != TOP_COLUMNS:
        raise AssertionError("eta6_p18 top header mismatch")

    witness = report.get("witness", {})
    if witness.get("classification") != "wide_grid_outside_222_screen":
        raise AssertionError("eta6_p18 classification mismatch")
    if witness.get("outside_min_spread") != 1164096:
        raise AssertionError("eta6_p18 min spread mismatch")
    if witness.get("outside_below_p15_unique_count") != 0:
        raise AssertionError("eta6_p18 below-p15 count mismatch")
    if witness.get("outside_support_equal_unique_count") != 0:
        raise AssertionError("eta6_p18 support equal count mismatch")
    ...
    if manifest.get("schema") != "eta6.p18.manifest@1":
        raise AssertionError("eta6_p18 manifest schema mismatch")
    if manifest.get("report_sha256") != report.get("certificate_sha256"):
        raise AssertionError("eta6_p18 manifest report hash mismatch")
    if self_hash(manifest, "manifest_sha256") != manifest.get("manifest_sha256"):
        raise AssertionError("eta6_p18 manifest self hash mismatch")
    ...
    if entry is None:
        raise AssertionError("eta6_p18 missing from index")
    if entry.get("report_sha256") != report.get("certificate_sha256"):
        raise AssertionError("eta6_p18 index report hash mismatch")
    if entry.get("status") != report.get("status"):
        raise AssertionError("eta6_p18 index status mismatch")
    index_without_hash = {
...
def main() -> None:
    print(json.dumps(validate_eta6_p18(), indent=2, sort_keys=True))
