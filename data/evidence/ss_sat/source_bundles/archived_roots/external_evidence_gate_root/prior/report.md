# G^natural Ontological Computation source_drop

## Scope

SOURCE_DROP builds the external solver/proof-log bridge. It probes the sandbox for third-party SAT solvers and DRAT/LRAT tools, adds runnable adapters, adds a small DIMACS benchmark suite, and certifies canary ingestion for solver stdout, DRAT-like, and LRAT-like inputs.
...
SOURCE_DROP's internally generated proof-certificate layer is carried forward as the last completed proof-log invariant. The source_drop contribution is the external reproducibility interface needed to ingest genuine MiniSat/CaDiCaL/Kissat-style solver logs or DRAT/LRAT proof files when supplied.

## How to run locally

```bash
cd gnatural_ontological_computation
