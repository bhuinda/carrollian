# cadical_drat_evidence — signed-Fano global action recovery test

## Question

Can the current source_drop/external_evidence_gate data certify the `(B+S)/V` chamber as the global signed-Fano payoff mechanism by recovering a signed-Fano action on the 16 builder and 16 jabber labels?
...
For each of the 168 elements of `GL(3,2) = Aut(F7)`, I transported the available Fano point distributions of the 16 builder and 16 jabber labels. Since source_drop only gives the three zones

```text
P1_point, L3_minus_P1, F7_minus_L3
```

I used the strongest possible reconstruction from source_drop alone: split each zone uniformly over its Fano points, then solve the optimal assignment back to the 16 labels by Hungarian matching. If a genuine 16-label signed-Fano action were already encoded in source_drop, this transport-and-match test would recover low-cost permutations beyond the incident flag stabilizer.
...
The external_evidence_gate six-channel result still stands: `(B+S)/V` is a real mechanism inside the Fano-nullified `6j` quotient shadow. But the current bundle cannot certify it as the full 16×16 signed-Fano payoff mechanism, because the available label data are only incident-flag zone aggregates.
