...
This isolates the first minimum-dimension open sector. It certifies the current raw-orbital matrix units and emits the GL2 normalization equation needed to compare a genuine legacy off-diagonal basis when one is supplied.
...
Next: Supply four candidate legacy sector-5 matrix units in the candidate schema, then solve for the GL2 variables and verify equality in raw-orbital coordinates.
